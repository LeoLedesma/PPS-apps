...
export class GTortaComponent  implements OnInit,OnDestroy {
  ...
  constructor(private gastosService: GastosService,private auth:AuthFirebaseService) { }
  ngOnDestroy(): void {
  ...
  traerObjetos() {
    this.gastosSub = this.gastosService.get(this.auth.userLogged.email).subscribe((informacion) => {
  ...
  createChart() {
    ...
    let categoriasA = this.gastos.movimientos.map(movimiento => movimiento.categoria);
